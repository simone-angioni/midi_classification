...
def classify(feature_engineering_techniques, log_name):
    logging.basicConfig(filename=f'{utils.base_dir}/results/{log_name}.log',format='%(asctime)s : %(message)s')
    try:
    ...
    folds_X, folds_y, dict1 = utils.load_clf_data_kfold(sample_len=initial_attention, fs=fs, feature_extraction=feature_engineering_techniques)
    ...
    for k in range(num_folds):
        ...
        model = tc.TransformerClassifier(feed_shape, vocabulary_size=12050)
